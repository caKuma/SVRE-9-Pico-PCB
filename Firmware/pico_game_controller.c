...
void joy_mode() {
  if (tud_hid_ready()) {
    ...
    report.joy0 = 0.7 * (report.joy0) + 0.3 * ((adc_read() * 255) / 4095);
    adc_select_input(1);
    report.joy1 = 0.7 * (report.joy1) + 0.3 * ((adc_read() * 255) / 4095);
    adc_select_input(0);
    } else {
...
void key_mode() {
  if (tud_hid_ready()) {  // Wait for ready, updating mouse too fast hampers
    ...
    if (kbm_report) {
      ...
      if (USE_POTENTIOMETERS) {
          int delta_pot[2] = {0};
          delta_pot[0] = (adc_read() * 255) / 4095 - report.joy0;
          adc_select_input(1);
          delta_pot[1] = (adc_read() * 255) / 4095 - report.joy1;
          adc_select_input(0);
          tud_hid_mouse_report(REPORT_ID_MOUSE, 0x00, delta_pot[0] * MOUSE_SENS, delta_pot[1] * MOUSE_SENS, 0, 0);
      } else {
...
void init() {
  ...
  if (USE_POTENTIOMETERS) {
    adc_init();
    adc_gpio_init(POT_GPIO_LEFT);
    adc_gpio_init(POT_GPIO_RIGHT);
    adc_select_input(0);
  }
